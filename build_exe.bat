...
if errorlevel 1 (
	gh release create "!RELEASE_TAG!" "dist\PCAP_Sentry\PCAP_Sentry.exe" --title "PCAP Sentry v%VERSION%" --notes "What's New: !BUILD_NOTES!" >> "%LOG_PATH%" 2>&1
	if errorlevel 1 (
	...
	gh release upload "!RELEASE_TAG!" "dist\PCAP_Sentry\PCAP_Sentry.exe" --clobber >> "%LOG_PATH%" 2>&1
	if errorlevel 1 (
